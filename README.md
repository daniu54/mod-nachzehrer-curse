...
Adds a **Cursed Knife** skill to the Preserver background. The wielder imbues a knife with cursed unholy energy and plunges it into any humanoid — ally, brother, or enemy — initiating their transformation into a Nachzehrer (ghoul). The knife is consumed in the ritual.
...
The **Cursed Knife** skill requires a knife or dagger equipped in the mainhand with remaining durability. The weapon is destroyed on use. On use:
...
In the MSU mod configuration menu (General page):

- **Enable Nachzehrer Curse perk tree**: adds/removes the perk from the Preserver background (requires game restart).
- **Require knife/dagger**: when enabled (default), the skill requires a knife or dagger with remaining durability in the mainhand. Disabling allows the skill to be used freely without a weapon.
